    ...
    <div className="report-page p-6 min-h-screen bg-slate-50">
      ...
      {report && (
        <div className="report-print-area max-w-2xl mx-auto bg-white rounded-xl border border-slate-200 shadow-sm p-8 text-slate-900">
          ...
          <ReportSection title="KEY METRICS · 关键指标"><div className="grid grid-cols-3 gap-3"><Metric label="总时长" value={`${(metrics.totalDurationMs / 1000).toFixed(1)}s`} /><Metric label="犹豫时长" value={`${(metrics.hesitationMs / 1000).toFixed(1)}s`} /><Metric label="压力峰值" value={session.hasFace ? metrics.peakStress.toFixed(2) : '无面部数据'} /><Metric label="方向反转" value={`${metrics.backAndForth || 0} 次`} /><Metric label="点击次数" value={`${metrics.totalClicks || 0} 次`} /><Metric label="最终决策" value={decisionLabel(metrics.finalDecision)} /></div></ReportSection>
